...
def bi_bfs_all(graph1: graph.Graph, start: str, end: str, max_path: int) -> list[list[str]] | bool:
    ...
    queue = deque([start_v])
    visited: set = {start_v}
    parent: dict[str: str | None] = {start_v: None}
    ...
    while (queue and queue_rev) and len(all_path) < max_path:
        ...
        for neighbor in node.forward_links:
            if neighbor not in visited:
                ...
                if neighbor in visited_rev:
                    path = _build_path(neighbor, parent, parent_rev)
                    if len(path) < size_of_shortest_path:  #make another since its the same below too
                        all_path.append(path)
                        size_of_shortest_path = len(path)
                        _check_current_list(all_path, size_of_shortest_path)  # will check if there are any path bigger than size of shortest
                    elif len(path) == size_of_shortest_path:
                        all_path.append(path)
                        _check_current_list(all_path, size_of_shortest_path)
...
g = load_graph.load_graph('wiki-topcats.txt', 'wiki-topcats-page-names.txt')
